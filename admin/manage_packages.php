...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_package'])) {
        ...
        $sql = "INSERT INTO packages (name, description, price, duration_days, inclusions, city_id, image_url, ideal_for, itinerary)
                VALUES ('$name', '$description', '$price', '$duration_days', '$inclusions', '$city_id', '$image_url', '$ideal_for', '$itinerary')";
...
<body>
    ...
    // Fetch packages data
    $sql = "SELECT package_id, name, price, duration_days, inclusions, city_id, image_url, ideal_for, itinerary, description FROM packages ORDER BY package_id DESC";
    $result = mysqli_query($conn, $sql);

    while ($row = mysqli_fetch_assoc($result)) {
        echo '<tr>';
        echo '<td>' . $row['package_id'] . '</td>';
        echo '<td><img src="../assets/images/packages/' . $row['image_url'] . '" class="package-img" alt="' . htmlspecialchars($row['name']) . '"></td>';
        echo '<td>' . htmlspecialchars($row['name']) . '</td>';
        echo '<td>₹' . htmlspecialchars($row['price']) . '</td>';
        echo '<td>' . htmlspecialchars($row['duration_days']) . ' days</td>';
        echo '<td class="ideal-for" title="' . htmlspecialchars($row['ideal_for']) . '">' . htmlspecialchars(substr($row['ideal_for'], 0, 30)) . '...</td>';
        echo '<td class="itinerary" title="' . htmlspecialchars($row['itinerary']) . '">' . htmlspecialchars(substr($row['itinerary'], 0, 50)) . '...</td>';
        echo '<td>' . (isset($cities[$row['city_id']]) ? htmlspecialchars($cities[$row['city_id']]) : 'N/A') . '</td>';
        echo '<td class="action-btns">';
        echo '<a href="#" class="edit-btn" onclick="openEditModal(' . $row['package_id'] . ', \'' . htmlspecialchars($row['name'], ENT_QUOTES) . '\', \'' . htmlspecialchars($row['description'], ENT_QUOTES) . '\', \'' . $row['price'] . '\', \'' . $row['duration_days'] . '\', \'' . htmlspecialchars($row['inclusions'], ENT_QUOTES) . '\', \'' . $row['city_id'] . '\', \'' . $row['image_url'] . '\', \'' . htmlspecialchars($row['itinerary'], ENT_QUOTES) . '\', \'' . htmlspecialchars($row['ideal_for'], ENT_QUOTES) . '\')"><i class="fas fa-edit"></i> Edit</a>';
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="modal_duration_days" name="duration_days" required
                        min="1">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="modal_ideal_for" name="ideal_for">
                </div>

                <div class="form-group">
                    <label for="itinerary">Itinerary</label>
                    <textarea class="form-control" id="modal_itinerary" name="itinerary" rows="5"></textarea>
                </div>
    ...
    <script>
        function openAddModal() {
            ...
            document.getElementById('modal_ideal_for').value = '';
            document.getElementById('modal_itinerary').value = '';
        ...
        function openEditModal(packageId, name, description, price, duration_days, inclusions, cityId, imageUrl, itinerary, idealFor) {
            ...
            document.getElementById('modal_ideal_for').value = idealFor;
            document.getElementById('modal_itinerary').value = itinerary;
